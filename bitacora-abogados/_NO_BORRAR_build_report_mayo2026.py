...
for row in rows_iter:
    ...
    if not es_str or es_str == 'CONCLUIDO':
        if es_str == 'CONCLUIDO' and is_dt(fe):
            _fe_co = to_dt(fe)
            _d_co = (REF - _fe_co).days
            if _d_co > 150:
                _ial_co = {'e':e,'ab':ab,'op':item_base['op'],'mu':item_base['mu'],
                           'fe':fe_str,'d':_d_co,'v':True,'xp':xp,'ac':ac_short}
                if is_ba:   al_bancos90.append({**_ial_co,'li':150})
                if is_info: al_info90.append({**_ial_co,'li':150})
                if is_fovi: al_fovi90.append({**_ial_co,'li':150})
        continue
    ...
    if is_tras and not is_dt(td_pago) and not excepcion_aplic and es_str != 'FALTA FINALIZAR':
    ...
    if es_str not in ('EN RPP', 'FALTA FINALIZAR') and not rpp_entered and d_nat > 45:
        al_rpp45.append({**item_al,'li':45})

    firmas_dt = row[H.get('ESC. Entrega Firmas Completas')]
    item_base['firmas_ok'] = is_dt(firmas_dt)   # campo para filtrar "Por Vencer" en template JS
    if not is_dt(firmas_dt) and d_nat > 60 and es_str != 'FALTA FINALIZAR':
    ...
    if not rpp_term and not is_ba and not is_info and not is_fovi and es_str != 'FALTA FINALIZAR':
        if is_tras and d_nat > 90:
            al_rpp90solo.append({**item_al,'li':90})
        elif not is_tras and d_nat > 30:
            al_rpp90solo.append({**item_al,'li':30})

    # FF usa is_*_seg (ignora acuse, solo acreedor) + criterio >150 días igual que el resto
    _is_ff = es_str == 'FALTA FINALIZAR'
    if (is_ba_seg if _is_ff else is_ba) and d_nat > 150:
        al_bancos90.append({**item_al,'li':150})
    if (is_info_seg if _is_ff else is_info) and d_nat > 150:
        al_info90.append({**item_al,'li':150})
    _fovi_ff = is_fovi_seg and (not is_ba_seg or _acuse_pend(_ba_acuse))
    if (_fovi_ff if _is_ff else is_fovi) and d_nat > 150:
        al_fovi90.append({**item_al,'li':150})

wb.close()

# ── Detectar abogado principal (el de mayor número de escrituras) ──
_primary_ab = max(ab_stats, key=lambda a: ab_stats[a]['total']) if ab_stats else 'ATM'
OUT = f"{_BASE_DL}/Bitacora {_primary_ab}.html"
print(f"{t()} Abogado detectado: {_primary_ab} → {OUT}")
...
# ── Tendencia: histórico del abogado detectado (2015-2025) + año en curso desde bitácora ──
_ab_hist = TEND_HIST.get(_primary_ab, TEND_HIST.get('TOTAL', {}))
data_me = {y: list(arr) for y, arr in sorted(_ab_hist.items())}
for y, arr in sorted(year_month_ab.get(_primary_ab, {}).items()):
    if y not in data_me:              # solo años no cubiertos por TEND_HIST (ej. 2026+)
        data_me[y] = list(arr)
print(f"{t()} Tendencia: {_primary_ab} — {len(data_me)} años de historial")
data_yr = {y: sum(arr) for y, arr in data_me.items()}
data_ab_me = {}
# 1) Historial TEND_HIST para cada abogado (2015-2025 hardcoded)
for ab_n, yr_dict in TEND_HIST.items():
    if ab_n == 'TOTAL':
        continue
    data_ab_me[ab_n] = {y: list(arr) for y, arr in yr_dict.items()}
# 2) Año en curso desde bitácora (solo años NO cubiertos por TEND_HIST, ej. 2026+)
for ab, yr_dict in year_month_ab.items():
    ab_n = 'LBR' if ab == 'COO' else ab
    if ab_n not in data_ab_me:
        data_ab_me[ab_n] = {}
    for y, arr in yr_dict.items():
        if y not in data_ab_me[ab_n]:
...
def replace_const(html, name, json_str):
    # Busca tanto 'const' como 'let' — el template usa 'let' para permitir reasignación desde localStorage
    pat = re.compile(rf'^(?:const|let) {re.escape(name)} = (\{{.*?\}}|\[.*?\]);\s*$', re.MULTILINE | re.DOTALL)
    new_line = f'let {name} = {json_str};'
...
html = re.sub(r'<title>[^<]+</title>', f'<title>Bitácora {_primary_ab} a {_ref_str}</title>', html, count=1)
html = re.sub(r'(<span id="h-ab-name">)[^<]*(</span>)', rf'\g<1>{_primary_ab}\g<2>', html, count=1)
html = re.sub(r'(<span id="rf-ab-name">)[^<]*(</span>)', rf'\g<1>{_primary_ab}\g<2>', html, count=1)
html = re.sub(r"(?:const|let) GH_REPO\s*=\s*'[^']*'", f"let GH_REPO = 'bitacora-{_primary_ab.lower()}'", html, count=1)
html = re.sub(r"(?:const|let) PRIMARY_AB\s*=\s*'[^']*'", f"let PRIMARY_AB = '{_primary_ab}'", html, count=1)
